...
const FName = people[3];
console.log(
  `The name of the 4th person is: {${FName.name.first} ${FName.name.middle} ${FName.name.last}}`
);

console.log(people[6].address.street);
const numberOfEligiblePeopleOver30 = people.filter(
  (person) =>
    person.age > 30 && !person.family.children.length && !person.family.spouse
).length;

console.log(
  `Number of people over 30 (excluding children and spouses): ${numberOfEligiblePeopleOver30}`
);
let totalAdults = 0;
let sumAdultAges = 0;

for (const person of people) {
  if (!person.family.spouse && person.family.children.length === 0) {
    totalAdults++;
    sumAdultAges += person.age;
  }
}

const averageAdultAge = totalAdults > 0 ? sumAdultAges / totalAdults : 0;

console.log(
  `Average age of adults (excluding children & spouses): ${averageAdultAge.toFixed(
    2
  )}`
);
